...
export const MAX_MANIFEST_BYTES = 32768;
/** A single argument value on the wire is clamped before typing. */
export const MAX_ARG_VALUE_LEN = 1024;
/**
 ...
 */
export function manifestFromEvent(event) {
 ...
 */
export function manifestToEvent(manifest, privateKeyBytes) {
...
export function botTag(botPublicKey) {
 ...
 */
export function addressedBots(tags) {
    const out = [];
 ...
 * quote.
 */
function nextToken(input, start) {
    let i = start;
 ...
 */
export function parseCommandText(manifest, content) {
 ...
 */
export function typedArgs(spec, parsed) {
    const out = new Map();
    const declared = spec.args ?? [];
